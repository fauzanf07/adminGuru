<?php
    ...
    if($productName == "AdminGuru Starter"){
        $amount = 0;
    }else if($productName == "AdminGuru Basic"){
        $amount = 500000;
    }else{
        $amount = 1000000;
    }
    ...
    if($num>=1){
        ...
        if($r['is_premium']==1){
            ...
            $order_id = rand();
            $sql = "INSERT INTO payment_history VALUES('','$order_id','$productName','$amount','$name','$email','$phone', '$city','$schoolName','0','','$date','0')";
            $result = mysqli_query($con,$sql);
            if($result){
